...
export default function IpamorelinCJC129510mgPage() {
	...
	const currentOriginalPrice = 115.99
				...
				<div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
					...
					<div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-12 items-start">
						...
						<div className="space-y-4">
							...
							<div>
								...
								<div className="flex items-center gap-2">
									<button type="button" onClick={(e) => { e.preventDefault(); e.stopPropagation(); setQuantity(Math.max(1, quantity - 1)); }} disabled={quantity === 1} className="w-12 h-12 rounded-lg border-2 border-border bg-card text-foreground flex items-center justify-center active:bg-primary/10 active:border-primary transition-colors disabled:opacity-50 disabled:cursor-not-allowed text-lg font-semibold touch-manipulation">-</button>
									<input type="number" value={quantity} onChange={(e) => setQuantity(Math.max(1, parseInt(e.target.value) || 1))} className="w-20 h-12 text-center border-2 border-border bg-card text-foreground rounded-lg font-medium text-sm focus:outline-none focus:border-primary" min="1" />
									<button type="button" onClick={(e) => { e.preventDefault(); e.stopPropagation(); setQuantity(quantity + 1); }} className="w-12 h-12 rounded-lg border-2 border-border bg-card text-foreground flex items-center justify-center active:bg-primary/10 active:border-primary transition-colors text-lg font-semibold touch-manipulation">+</button>
								</div>
							</div>
							<button type="button" onClick={handleAddToCart} className="w-full py-3 px-4 rounded-lg font-medium text-sm transition-colors flex items-center justify-center gap-2 bg-primary text-primary-foreground hover:bg-primary/90 shadow-md hover:shadow-lg">
							...
							<div className="grid grid-cols-1 gap-2 pt-3 border-t border-border">
								...
								<div className="flex items-center gap-2 p-2 rounded-lg bg-secondary/50 hover:bg-secondary transition-colors">
									<div className="bg-primary/10 p-1.5 rounded-md"><Award className="h-4 w-4 text-primary" /></div>
									<p className="text-xs font-medium text-foreground">High grade purity Premium quality</p>
								</div>
								<div className="flex items-center gap-2 p-2 rounded-lg bg-secondary/50 hover:bg-secondary transition-colors">
									<div className="bg-primary/10 p-1.5 rounded-md"><Headphones className="h-4 w-4 text-primary" /></div>
									<p className="text-xs font-medium text-foreground">Customer Happiness 100% Guaranteed</p>
								</div>
								<div className="flex items-center gap-2 p-2 rounded-lg bg-secondary/50 hover:bg-secondary transition-colors">
									<div className="bg-primary/10 p-1.5 rounded-md"><Lock className="h-4 w-4 text-primary" /></div>
					...
					<div className="mt-16 grid grid-cols-1 lg:grid-cols-2 gap-4 sm:p-8 lg:gap-12">
						...
						<div className="bg-card rounded-2xl p-4 sm:p-8 border border-border shadow-sm">
							...
							<p className="text-muted-foreground leading-relaxed">This peptide blend contains synthetic CJC-1295 (no DAC) and synthetic Ipamorelin, 10 mg each, prepared under controlled laboratory conditions. The components are defined amino acid sequences manufactured for analytical characterization and experimental laboratory applications.</p>
						</div>
					</div>
