...
export default function GHKCuPage() {
	...
	const mgOptions = [
		...
		{ value: '50mg', price: 34.99, originalPrice: 38.99, image: '/products/ghk-cu/GHK-Cu 50mg.png', inStock: false },
	]

	const currentOption = mgOptions.find(option => option.value === selectedMG)
	const currentPrice = currentOption?.price || 48.99
	...
	return (
		<div className="min-h-screen bg-background">
			<Header />
			<ProductNavigation currentProductId="ghk-cu" />
					...
					<div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-12 items-start">
						...
						<div className="lg:sticky lg:top-28">
							<ProductImageCarousel
								slides={
									selectedMG === '50mg'
										? [
												{ src: currentImage, alt: `GHK-Cu ${selectedMG}` },
												{ src: '/COAs/3rd party testing/GHK-Cu 50mg.JPG', alt: 'GHK-Cu 50mg Certificate of Analysis', isCoa: true },
											]
										: [{ src: currentImage, alt: `GHK-Cu ${selectedMG}` }]
								}
								priority
						...
						<div className="space-y-4 sm:space-y-6">
							...
							<div>
								...
								<div className="flex items-center gap-3">
									...
									<input
										...
										className="w-20 h-12 text-center border-2 border-border bg-card text-foreground rounded-lg font-semibold text-base focus:outline-none focus:border-primary touch-manipulation"
										min="1"
									/>
									<button
										type="button"
							...
							<button
								...
								disabled={!isInStock}
								className={`w-full py-3 px-4 rounded-lg font-medium text-sm transition-colors flex items-center justify-center gap-2 touch-manipulation ${
									isInStock
						...
						<div className="bg-card rounded-2xl p-4 sm:p-8 border border-border shadow-sm">
							...
							<div className="space-y-4">
								<div className="flex justify-between py-2 border-b border-border/50">
									...
									<span className="text-muted-foreground">Blue powder</span>
								</div>
								<div className="flex justify-between py-2 border-b border-border/50">
									<span className="font-medium text-foreground">Cas No:</span>
									<span className="text-muted-foreground">130120-57-9</span>
								</div>
								<div className="flex justify-between py-2 border-b border-border/50">
									<span className="font-medium text-foreground">Molecular Formula:</span>
									<span className="text-muted-foreground">C<sub>14</sub>H<sub>24</sub>N<sub>6</sub>O<sub>4</sub></span>
								</div>
								<div className="flex justify-between py-2 border-b border-border/50">
					...
					<div className="mt-16">
						...
						<div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4">
							{[
								...
								{ id: 'mix-peptide', name: 'GLOW 70mg', price: 88.99, originalPrice: 98.99, image: '/products/bpc-tb-ghk-mix/GLOW 70mg.png' },
							].map((product) => (
